...
export function importIngredients(
  row: Pick<CompanyImportRow | CentralImportRow, 'ingredientName' | 'strengthValue' | 'strengthUnit' | 'ingredients'>,
): ImportIngredientRow[] {
  const supplied = row.ingredients !== undefined
    ? row.ingredients
    : row.ingredientName
...
export async function importCompanyCombinationRow(
  ...
  const compositionProvided = row.ingredients !== undefined || Boolean(optional(row.ingredientName));
  ...
  return db.$transaction(async (tx) => {
    ...
    const replay = await tx.product.findFirst({
      where: { companyId, createRequestKey: replayKey },
      select: { id: true },
    });
    ...
    if (mode === 'UPSERT_PRODUCT_CODE' && productCode) {
      existingId = (await tx.product.findFirst({
        where: { companyId, productCode },
        select: { id: true },
      }))?.id ?? null;
    }
    if (mode === 'UPSERT_BARCODE' && barcode) {
      existingId = (await tx.productBarcode.findFirst({
        where: { companyId, value: barcode },
    ...
    const manufacturerId = row.manufacturerName
      ? (await upsertMaster(tx, 'manufacturer', companyId, row.manufacturerName)).id
      : undefined;
    const categoryId = row.categoryName
      ? (await upsertMaster(tx, 'category', companyId, row.categoryName)).id
      : undefined;
    const dosageFormId = row.dosageFormName
      ? (await upsertMaster(tx, 'dosageForm', companyId, row.dosageFormName)).id
      : undefined;
    const routeId = row.routeName
      ? (await upsertMaster(tx, 'route', companyId, row.routeName)).id
      : undefined;
    const ingredientAssignments = await resolveIngredientAssignments(tx, companyId, ingredients);

    if (existingId) {
      const current = await tx.product.findFirst({
        where: { id: existingId, companyId },
      ...
      if (row.regulatoryId !== undefined && row.regulatoryId !== null && row.regulatoryId.trim()) {
        data.regulatoryId = row.regulatoryId.trim().toUpperCase();
      }
      if (row.atcCode !== undefined && row.atcCode !== null && row.atcCode.trim()) {
        data.atcCode = row.atcCode.trim().toUpperCase();
      }
      ...
      if (row.referencePrice !== undefined && row.referencePrice !== null && row.referencePrice.trim()) {
        data.referencePrice = decimal(row.referencePrice, 'Reference price');
      }
      ...
      const replaceComposition = row.productType === 'NON_DRUG' || compositionProvided;
      if (replaceComposition) {
        ...
        if (ingredientAssignments.length) {
          await tx.productIngredientAssignment.createMany({
            data: ingredientAssignments.map((ingredient) => ({ productId: existingId!, ...ingredient })),
          });
        }
      }
      await upsertImportPackaging(tx, companyId, existingId, row, barcode);
      if (current.status === 'ACTIVE' && row.productType === 'DRUG' && replaceComposition && ingredientAssignments.length === 0) {
    ...
    if (ingredientAssignments.length) {
      await tx.productIngredientAssignment.createMany({
        data: ingredientAssignments.map((ingredient) => ({ productId: id, ...ingredient })),
      });
    }
  ...
  return db.$transaction(async (tx) => {
    ...
    const regulatoryConflict = row.regulatoryId
      ? await tx.centralDrugReference.findFirst({
          where: { regulatoryId: row.regulatoryId, id: { not: existing?.id } },
          select: { id: true },
        })
      : null;
    ...
    const barcodeConflict = barcodeValue
      ? await tx.centralDrugBarcode.findFirst({
          where: { value: barcodeValue, centralDrugId: { not: existing?.id } },
          select: { id: true },
        })
      : null;
    const reason = [
    ...
    if (existing) {
      await tx.centralDrugReference.update({
    ...
    if (!existing || compositionProvided) {
    ...
    if (barcodeValue && !barcodeConflict) {
      await tx.centralDrugBarcode.create({
        data: {
          centralDrugId: centralId,
          value: barcodeValue,
          symbology: optional(row.barcodeSymbology) ?? null,
        },
      });
    }
    return {
...
async function resolveIngredientAssignments(tx: Tx, companyId: string, ingredients: ImportIngredientRow[]) {
...
async function upsertMaster(
  ...
  switch (kind) {
    case 'manufacturer':
      return tx.productManufacturer.upsert({
        where: { companyId_normalizedName: { companyId, normalizedName } },
        create: { companyId, name, normalizedName }, update: { active: true }, select: { id: true },
      });
    case 'category':
    ...
    case 'dosageForm':
      return tx.productDosageForm.upsert({
        where: { companyId_normalizedName: { companyId, normalizedName } },
        create: { companyId, name, normalizedName }, update: { active: true }, select: { id: true },
      });
    case 'route':
      return tx.productRoute.upsert({
        where: { companyId_normalizedName: { companyId, normalizedName } },
        create: { companyId, name, normalizedName }, update: { active: true }, select: { id: true },
      });
    case 'ingredient':
      return tx.productIngredientMaster.upsert({
        where: { companyId_normalizedName: { companyId, normalizedName } },
        create: { companyId, name, normalizedName }, update: { active: true }, select: { id: true },
      });
  }
...
async function upsertImportPackaging(
  tx: Tx,
  companyId: string,
  productId: string,
  row: CompanyImportRow,
  ...
  if (baseName) {
    if (base) {
      base = await tx.productUnit.update({
        where: { id: base.id },
        data: { name: baseName, version: { increment: 1 } },
      });
    } else {
  ...
  if (packageName) {
    ...
    if (!factor || factor.lessThanOrEqualTo(0)) {
      throw new ProductClassificationError('Package conversion factor must be greater than zero');
    }
    target = await tx.productUnit.findFirst({ where: { productId, name: packageName, active: true } })
      ?? await tx.productUnit.create({
        data: { productId, name: packageName, conversionFactor: factor, defaultPurchase: true },
      });
  }
  if (barcode) {
    ...
    const existing = await tx.productBarcode.findUnique({
      where: { companyId_value: { companyId, value: barcode } },
    });
    if (existing && existing.productId !== productId) {
      throw new ProductPersistenceConflictError('Barcode belongs to another product');
    }
    if (existing) {
      await tx.productBarcode.update({
        ...
        data: {
          unitId: target.id,
          symbology: optional(row.barcodeSymbology) ?? existing.symbology,
          active: true,
          version: { increment: 1 },
        },
      });
...
function decimal(value: string | null | undefined, label: string): Prisma.Decimal | null {
  ...
  if (!raw) return null;
  if (!/^\d+(?:\.\d{1,6})?$/.test(raw)) {
    throw new ProductClassificationError(`${label} must be a non-negative decimal`);
  }
