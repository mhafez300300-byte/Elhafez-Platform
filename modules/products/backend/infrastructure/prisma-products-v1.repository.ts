...
function mapCategory(row: ProductRow['category'] extends infer T ? NonNullable<T> : never): ProductCategoryView {
  return {
    id: row.id,
...
function mapIngredient(row: ProductRow['ingredients'][number]): ProductIngredientView {
  return {
    id: row.id,
    productId: row.productId,
    ingredient: mapSimple(row.ingredient),
    strengthValue: row.strengthValue?.toString() ?? null,
...
function mapSummary(row: ProductRow): ProductSummary {
  ...
  return {
    ...
    productType: row.productType as ProductSummary['productType'],
    ...
    centralReferenceId: row.centralReferenceId,
...
function mapDetail(row: ProductRow): ProductDetail {
  return {
    ...
    ingredients: row.ingredients.map(mapIngredient),
    units: row.units.map(mapUnit),
    barcodes: row.barcodes.map(mapBarcode),
    centralSnapshot: row.centralSnapshot,
  };
}
...
function mapSession(row: {
  id: string; kind: string; scopeKey: string; companyId: string | null; mode: string; status: string; sourceName: string | null; sourceId: string | null; datasetKey: string | null; totalRows: number | null; processedRows: number; acceptedRows: number; updatedRows: number; rejectedRows: number; quarantinedRows: number; createdBy: string; createdAt: Date; updatedAt: Date;
...
function normalizeCompanyImportRow(row: CompanyImportRow): CompanyImportRow {
  return {
    ...
    arabicName: optionalText(row.arabicName),
    englishName: optionalText(row.englishName),
    tradeName: optionalText(row.tradeName),
    manufacturerName: optionalText(row.manufacturerName),
    categoryName: optionalText(row.categoryName),
    ingredientName: optionalText(row.ingredientName),
    strengthValue: optionalText(row.strengthValue),
    strengthUnit: optionalText(row.strengthUnit),
    dosageFormName: optionalText(row.dosageFormName),
    routeName: optionalText(row.routeName),
    barcode: optionalText(row.barcode),
    barcodeSymbology: optionalText(row.barcodeSymbology),
    baseUnitName: optionalText(row.baseUnitName),
    packageUnitName: optionalText(row.packageUnitName),
    packageFactor: optionalText(row.packageFactor),
    regulatoryId: optionalText(row.regulatoryId),
    atcCode: optionalText(row.atcCode),
    prescriptionClass: optionalText(row.prescriptionClass),
    referencePrice: optionalText(row.referencePrice),
    referencePriceSource: optionalText(row.referencePriceSource),
...
export class PrismaProductsV1Repository extends PrismaProductsRepository implements ProductsHardeningRepository {
  ...
  override async list(query: ProductListQuery): Promise<ProductListResult> {
    const filter = await this.productWhere(query, false);
    if (query.search && query.page === 1) {
      const barcode = normalizeBarcodeSafe(query.search);
      const exact = await this.db.productBarcode.findFirst({ where: { companyId: query.companyId, value: barcode, active: true }, select: { productId: true } });
      if (exact) {
        const row = await this.db.product.findFirst({ where: { ...filter, id: exact.productId }, include: includeProduct });
  ...
  override async exportList(query: Omit<ProductListQuery, 'page' | 'pageSize'>): Promise<ProductDetail[]> {
    const where = await this.productWhere({ ...query, page: 1, pageSize: 100 }, true);
    const rows = await this.db.product.findMany({ where, include: includeProduct, orderBy: this.orderBy({ ...query, page: 1, pageSize: 100 }), take: 100000 });
  ...
  override async claimImportChunk(sessionId: string, chunkKey: string): Promise<ImportChunkClaim> {
    ...
    const canRecover = current.status === 'FAILED' || (current.status === 'PROCESSING' && current.updatedAt <= staleBefore);
    if (!canRecover) {
      return { id: current.id, status: current.status, result: null, claimed: false, recovered: false };
    }
    const updated = await this.db.productImportChunk.updateMany({
      where: { id: current.id, status: current.status, updatedAt: current.updatedAt },
      data: { status: 'PROCESSING', result: Prisma.JsonNull },
    });
    if (updated.count !== 1) return this.claimImportChunk(sessionId, chunkKey);
    await this.db.productImportSession.update({ where: { id: sessionId }, data: { status: 'PROCESSING' } });
  ...
  override async importCompanyRow(sessionId: string, mode: string, row: CompanyImportRow) {
    ...
    if (normalized.ingredients && normalized.ingredients.length > 1) {
      throw new ProductClassificationError('Multi-ingredient spreadsheet rows must use the explicit multi-ingredient import format');
    }
  ...
  async listCompanyImportSessions(companyId: string, limit: number): Promise<ImportSessionAdminView[]> {
    const rows = await this.db.productImportSession.findMany({ where: { kind: 'COMPANY', companyId }, orderBy: [{ createdAt: 'desc' }, { id: 'asc' }], take: Math.min(Math.max(limit, 1), 100) });
    return rows.map(mapSession);
  }

  async listCentralImportSessions(limit: number): Promise<ImportSessionAdminView[]> {
    const rows = await this.db.productImportSession.findMany({ where: { kind: 'CENTRAL', scopeKey: 'GLOBAL' }, orderBy: [{ createdAt: 'desc' }, { id: 'asc' }], take: Math.min(Math.max(limit, 1), 100) });
    return rows.map(mapSession);
  }

  async listCentralDatasets(limit: number): Promise<CentralDatasetView[]> {
    const rows = await this.db.centralDrugDataset.findMany({ include: { source: true }, orderBy: [{ ingestedAt: 'desc' }, { id: 'asc' }], take: Math.min(Math.max(limit, 1), 100) });
    return rows.map((row) => ({
      id: row.id,
      sourceId: row.sourceId,
  ...
  async getCentralAdmin(referenceId: string): Promise<CentralReferenceAdminView | null> {
    const row = await this.db.centralDrugReference.findUnique({
      ...
      include: { ...includeCentral, source: true, dataset: { include: { source: true } } },
    });
    if (!row) return null;
    const dataset = row.dataset ? {
      ...
      source: {
        id: row.dataset.source.id,
        sourceKey: row.dataset.source.sourceKey,
        name: row.dataset.source.name,
        provenance: row.dataset.source.provenance,
        licenseNote: row.dataset.source.licenseNote,
        sourceUrl: row.dataset.source.sourceUrl,
        active: row.dataset.source.active,
      },
    } satisfies CentralDatasetView : null;
  ...
  async governCentralReference(referenceId: string, input: CentralGovernanceInput): Promise<CentralReferenceAdminView> {
    ...
    await this.db.$transaction(async (tx) => {
      ...
      const allowed: Record<string, readonly string[]> = {
        ...
        RETIRED: ['ACTIVE'],
        QUARANTINED: ['ACTIVE', 'SUPERSEDED', 'RETIRED'],
      };
      ...
      if (input.status === 'ACTIVE') {
        if (current.regulatoryId) {
          const conflict = await tx.centralDrugReference.findFirst({ where: { id: { not: current.id }, regulatoryId: current.regulatoryId, status: 'ACTIVE' }, select: { id: true } });
        ...
        for (const barcode of current.barcodes) {
          const conflict = await tx.centralDrugBarcode.findFirst({ where: { value: barcode.value, centralDrugId: { not: current.id } }, select: { id: true } });
  ...
  async previewBulk(companyId: string, productIds: string[], patch: BulkProductPatch): Promise<BulkPreviewResult> {
    ...
    const rows = await this.db.product.findMany({ where: { companyId, id: { in: ids } }, include: { units: true, ingredients: true, tags: true } });
    ...
    return {
      ...
      issues: [...missing.map((productId) => ({ productId, code: 'NOT_FOUND', message: 'Product not found in authorized company' }) satisfies BulkPreviewIssue), ...issues],
    };
  }
  ...
  async executeBulk(companyId: string, productIds: string[], patch: BulkProductPatch, idempotencyKey: string, actorId: string): Promise<BulkExecutionResult> {
    ...
    if (!this.hasBulkPatch(patch)) throw new ProductClassificationError('Bulk update requires at least one explicitly selected field');
    const canonical = { productIds: ids, patch: { ...patch, tagIds: patch.tagIds ? [...new Set(patch.tagIds)].sort() : undefined } };
    const requestHash = createHash('sha256').update(JSON.stringify(canonical)).digest('hex');
    try {
      const changed = await this.db.$transaction(async (tx) => {
        await tx.productImportSession.create({
          data: {
            kind: 'BULK',
            scopeKey: companyId,
            companyId,
            ...
            mode: 'BULK_UPDATE_V1',
            status: 'PROCESSING',
            sourceName: 'Products bulk update',
            mapping: { requestHash, patch: canonical.patch } as Prisma.InputJsonValue,
        ...
        for (const row of rows) {
          ...
          if (Object.prototype.hasOwnProperty.call(patch, 'status') && patch.status) data.status = patch.status;
          ...
          if (Object.prototype.hasOwnProperty.call(patch, 'marketStatus') && patch.marketStatus) data.marketStatus = patch.marketStatus;
          ...
          if (patch.tagIds) {
            ...
            if (patch.tagIds.length) await tx.productTagAssignment.createMany({ data: [...new Set(patch.tagIds)].map((tagId) => ({ productId: row.id, tagId })) });
          }
          changedCount += 1;
        }
      ...
      const mapping = existing.mapping && typeof existing.mapping === 'object' && !Array.isArray(existing.mapping) ? existing.mapping as Record<string, unknown> : {};
  ...
  override async importCentralRow(_sessionId: string, context: CentralImportContext, row: CentralImportRow) {
    try {
      return await this.db.$transaction(async (tx) => {
        ...
        const existing = await tx.centralDrugReference.findUnique({ where: { sourceId_sourceRecordKey: { sourceId: context.sourceId, sourceRecordKey: row.sourceRecordKey } }, include: includeCentral });
        const regulatoryConflict = row.regulatoryId ? await tx.centralDrugReference.findFirst({ where: { regulatoryId: row.regulatoryId, id: { not: existing?.id } } }) : null;
        const barcodeValue = row.barcode ? normalizeBarcode(row.barcode) : null;
        const barcodeConflict = barcodeValue ? await tx.centralDrugBarcode.findFirst({ where: { value: barcodeValue, centralDrugId: { not: existing?.id } } }) : null;
        ...
        const data = {
          ...
          canonicalName: row.canonicalName,
          normalizedName: normalizeSearchText(row.canonicalName),
          arabicName: optionalText(row.arabicName) ?? null,
          normalizedArabicName: optionalText(row.arabicName) ? normalizeSearchText(optionalText(row.arabicName)!) : null,
          englishName: optionalText(row.englishName) ?? null,
          normalizedEnglishName: optionalText(row.englishName) ? normalizeSearchText(optionalText(row.englishName)!) : null,
          tradeName: optionalText(row.tradeName) ?? null,
          normalizedTradeName: optionalText(row.tradeName) ? normalizeSearchText(optionalText(row.tradeName)!) : null,
          manufacturerName: optionalText(row.manufacturerName) ?? null,
          dosageForm: optionalText(row.dosageForm) ?? null,
          route: optionalText(row.route) ?? null,
          regulatoryId: regulatoryConflict ? null : optionalText(row.regulatoryId) ?? null,
          atcCode: optionalText(row.atcCode) ?? null,
          prescriptionClass: optionalText(row.prescriptionClass) ?? null,
          controlled: row.controlled ?? false,
          packageDescription: optionalText(row.packageDescription) ?? null,
          baseUnitLabel: optionalText(row.baseUnitLabel) ?? null,
          packageUnitLabel: optionalText(row.packageUnitLabel) ?? null,
          conversionFactor: row.conversionFactor ? new Prisma.Decimal(row.conversionFactor) : null,
          referencePrice: row.referencePrice ? new Prisma.Decimal(row.referencePrice) : null,
        ...
        let central;
        if (existing) {
          central = await tx.centralDrugReference.update({ where: { id: existing.id }, data: { ...data, version: { increment: 1 } } });
          await tx.centralDrugIngredient.deleteMany({ where: { centralDrugId: existing.id } });
          await tx.centralDrugBarcode.deleteMany({ where: { centralDrugId: existing.id } });
        } else {
          central = await tx.centralDrugReference.create({ data: { sourceId: context.sourceId, sourceRecordKey: row.sourceRecordKey, ...data } });
        }
        const ingredientRows = row.ingredients?.length
          ? row.ingredients
          : row.ingredientName
        ...
        for (let index = 0; index < ingredientRows.length; index += 1) {
          const ingredient = ingredientRows[index]!;
          const name = cleanText(ingredient.name, 180);
          if (!name) throw new ProductClassificationError('Central ingredient name is required');
          await tx.centralDrugIngredient.create({
            data: {
              centralDrugId: central.id,
              name,
              normalizedName: normalizeSearchText(name),
              strengthValue: ingredient.strengthValue ? new Prisma.Decimal(ingredient.strengthValue) : null,
              strengthUnit: cleanText(ingredient.strengthUnit, 40),
              sortOrder: index,
            },
          });
        }
        if (barcodeValue && !barcodeConflict) await tx.centralDrugBarcode.create({ data: { centralDrugId: central.id, value: barcodeValue, symbology: optionalText(row.barcodeSymbology) ?? null } });
        const full = await tx.centralDrugReference.findUnique({ where: { id: central.id }, include: includeCentral });
        if (!full) throw new ProductPersistenceConflictError('Central record disappeared during ingestion');
        return { status: (reason ? 'QUARANTINED' : existing ? 'UPDATED' : 'ACCEPTED') as 'ACCEPTED' | 'UPDATED' | 'QUARANTINED', central: mapCentral(full), reason: reason ?? undefined };
      });
    } catch (error) {
      if (error instanceof ProductClassificationError || error instanceof ProductConcurrentUpdateError || error instanceof ProductPersistenceConflictError || error instanceof ProductDomainError) throw error;
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') throw new ProductPersistenceConflictError('Central strong identifier already exists');
      throw error;
    }
  }
  ...
  private orderBy(query: Pick<ProductListQuery, 'sortBy' | 'sortDirection'>): Prisma.ProductOrderByWithRelationInput[] {
    const direction = query.sortDirection ?? (query.sortBy === 'NAME' || query.sortBy === 'PRODUCT_CODE' || query.sortBy === 'MANUFACTURER' || query.sortBy === 'CATEGORY' ? 'asc' : 'desc');
  ...
  private async categoryWithDescendants(companyId: string, rootId: string): Promise<string[]> {
    ...
    if (!available.has(rootId)) return [rootId];
  ...
  private hasBulkPatch(patch: BulkProductPatch): boolean {
    return ['categoryId', 'manufacturerId', 'tagIds', 'status', 'prescriptionClass', 'marketStatus', 'controlled', 'coldChain'].some((key) => Object.prototype.hasOwnProperty.call(patch, key));
  }

  private async validateBulkReferences(client: Db, companyId: string, patch: BulkProductPatch): Promise<void> {
    if (patch.categoryId) {
      const count = await client.productCategory.count({ where: { id: patch.categoryId, companyId, active: true } });
      if (count !== 1) throw new ProductClassificationError('Bulk category is invalid for authorized company');
    }
    if (patch.manufacturerId) {
      const count = await client.productManufacturer.count({ where: { id: patch.manufacturerId, companyId, active: true } });
      if (count !== 1) throw new ProductClassificationError('Bulk manufacturer is invalid for authorized company');
    }
    if (patch.tagIds) {
      const ids = [...new Set(patch.tagIds)];
      const count = await client.productTag.count({ where: { id: { in: ids }, companyId, active: true } });
      if (count !== ids.length) throw new ProductClassificationError('One or more bulk tags are invalid for authorized company');
    }
  }
  ...
  private bulkIssues(row: { id: string; status: string; productType: string; units: Array<{ active: boolean; isBase: boolean }>; ingredients: unknown[] }, patch: BulkProductPatch): BulkPreviewIssue[] {
    ...
    const changesMasterData = ['categoryId', 'manufacturerId', 'tagIds', 'prescriptionClass', 'marketStatus', 'controlled', 'coldChain'].some((key) => Object.prototype.hasOwnProperty.call(patch, key));
    if (row.status === 'ARCHIVED' && changesMasterData && (!patch.status || patch.status === 'ARCHIVED')) {
    ...
    if (patch.status && patch.status !== row.status) {
      try {
        assertStatusTransition(row.status as ProductSummary['status'], patch.status);
      } catch (error) {
        issues.push({ productId: row.id, code: 'INVALID_STATUS_TRANSITION', message: error instanceof Error ? error.message : 'Invalid status transition' });
      }
    }
  ...
  private bulkRowWouldChange(row: { categoryId?: string | null; manufacturerId?: string | null; status: string; prescriptionClass?: string | null; marketStatus?: string; controlled?: boolean; coldChain?: boolean; tags: Array<{ tagId: string }> }, patch: BulkProductPatch): boolean {
