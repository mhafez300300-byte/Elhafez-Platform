...
import type { AccessPrincipal, AuditRequestedEvent } from '@elhafez/platform-contracts';
...
export class AuthService {
  ...
  async login(login: string, password: string): Promise<AuthTokens> {
    ...
    await this.repo.createSession({
      id: sid,
      userId: user.id,
      refreshTokenHash: this.tokenHash(refresh),
      expiresAt: new Date(Date.now() + this.tokens.refreshTtl * 1000),
    });
    return {
      accessToken: access,
      refreshToken: refresh,
      accessExpiresInSeconds: this.tokens.accessTtl,
      refreshExpiresInSeconds: this.tokens.refreshTtl,
    };
  }
  ...
  async refresh(refreshToken: string): Promise<AuthTokens> {
    ...
    if (
      ...
      throw new AuthenticationError('Refresh session is invalid');
    }
    ...
    const access = this.tokens.signAccess({ ...nextBase, typ: 'access' });
    const refresh = this.tokens.signRefresh({ ...nextBase, typ: 'refresh' });
    const rotated = await this.repo.rotateSession(payload.sid, currentHash, {
      refreshTokenHash: this.tokenHash(refresh),
      expiresAt: new Date(Date.now() + this.tokens.refreshTtl * 1000),
    });
    if (!rotated) throw new AuthenticationError('Refresh token was already consumed');
  ...
  async verifyAccess(token: string): Promise<AccessPrincipal> {
    ...
    if (!session || session.revokedAt || session.userId !== payload.sub) {
      throw new AuthenticationError('Session is revoked');
    }
    const user = await this.users.getIdentity(payload.sub);
    if (!user || user.status !== 'ACTIVE' || user.tokenVersion !== payload.tokenVersion) {
      throw new AuthenticationError('User session is no longer valid');
  ...
  async setCredential(
    input: { userId: string; login: string; password: string },
    actorId?: string,
  ): Promise<CredentialView> {
    const user = await this.users.getIdentity(input.userId);
    if (!user) throw new NotFoundError('User not found');
    const login = input.login.toLowerCase();
    const existing = await this.repo.findCredentialByLogin(login);
    if (existing && existing.userId !== input.userId) throw new ConflictError('Login already used');

    const row = await this.repo.upsertCredential({
      userId: input.userId,
      login,
      passwordHash: await this.passwords.hash(input.password),
    });
    await this.repo.revokeAllUserSessions(input.userId);
    const audit: AuditRequestedEvent = {
      type: 'platform.audit.requested',
      occurredAt: new Date().toISOString(),
      actorId,
      entityType: 'auth-credential',
      entityId: input.userId,
      action: 'auth.credential.set',
      metadata: { login },
    };
    await this.events.publish(audit);
    return {
      userId: row.userId,
      login: row.login,
