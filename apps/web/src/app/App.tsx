...
export function App() {
  ...
  return <div className="shell"><aside><div className="brand">Elhafez Platform</div><p>Core + Customers v1 + Suppliers v1</p><button className={active==='Customers'?'active':''} onClick={()=>{setActive('Customers');setData(null);setError(null);}}>Customers</button><button className={active==='Suppliers'?'active':''} onClick={()=>{setActive('Suppliers');setData(null);setError(null);}}>Suppliers</button>{sections.map(([name,path])=><button key={name} className={active===name?'active':''} onClick={()=>void load(name,path)}>{name}</button>)}<button onClick={signOut}>Logout</button></aside><main>{active==='Customers'?<CustomersWorkspace baseUrl={baseUrl} accessToken={accessToken}/>:active==='Suppliers'?<SuppliersWorkspace baseUrl={baseUrl} accessToken={accessToken}/>:<><h1>{active}</h1><p className="muted">Approved platform Core administration.</p>{error&&<p className="error">{error}</p>}<pre>{data===null?'Select a Core section to load live data.':JSON.stringify(data,null,2)}</pre></>}</main></div>;
}
