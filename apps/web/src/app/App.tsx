...
import { CustomersWorkspace } from '@elhafez/customers/frontend';
import { SuppliersWorkspace } from '@elhafez/suppliers/frontend';
import { ProductsWorkspace } from '@elhafez/products/frontend';
...
export function App() {
  ...
  function openWorkspace(name:'Customers'|'Suppliers'|'Products'){setActive(name);setData(null);setError(null);}
  ...
  return <div className="shell"><aside><div className="brand">Elhafez Platform</div><p>Core + Customers v1 + Suppliers v1 + Products v1</p><button className={active==='Customers'?'active':''} onClick={()=>openWorkspace('Customers')}>Customers</button><button className={active==='Suppliers'?'active':''} onClick={()=>openWorkspace('Suppliers')}>Suppliers</button><button className={active==='Products'?'active':''} onClick={()=>openWorkspace('Products')}>Products</button>{sections.map(([name,path])=><button key={name} className={active===name?'active':''} onClick={()=>void load(name,path)}>{name}</button>)}<button onClick={signOut}>Logout</button></aside><main>{active==='Customers'?<CustomersWorkspace baseUrl={baseUrl} accessToken={accessToken}/>:active==='Suppliers'?<SuppliersWorkspace baseUrl={baseUrl} accessToken={accessToken}/>:active==='Products'?<ProductsWorkspace baseUrl={baseUrl} accessToken={accessToken}/>:<><h1>{active}</h1><p className="muted">Approved platform Core administration.</p>{error&&<p className="error">{error}</p>}<pre>{data===null?'Select a Core section to load live data.':JSON.stringify(data,null,2)}</pre></>}</main></div>;
}
